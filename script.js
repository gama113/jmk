...
document.addEventListener("DOMContentLoaded", function() {
  ...
  const dropdownButton = document.querySelector(".dropbtn");
  const dropdownContent = document.querySelector(".dropdown-content");
  const sidebarLinks = document.querySelectorAll("#mySidebar_sidepanel a"); // Select all links in sidebar
  ...
  sidebarLinks.forEach(link => {
    link.addEventListener("click", function(event) {
      if (link.getAttribute("href") === "#contact_us") {
        const contactUsElement = document.getElementById("contact_us");
        if (contactUsElement) {
          event.preventDefault(); // Prevent default anchor behavior
          contactUsElement.scrollIntoView({ behavior: "smooth" });
        }
      }
      sidebar.style.width = "0"; // Close the sidebar after any link click.
    });
  });
});
////////////////////////////////////////////////////////////
